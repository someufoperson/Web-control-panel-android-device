...
async def list_device_kb(list_device: list):
    ...
    for device in list_device:
        status_device = (
            "🟢онлайн" if device["status_device"] == "ONLINE" else "🔴офлайн"
        )
        session_status = (
            "🟢сессия" if device["session_status"] == "ACTIVE" else "🔴сессия"
        )
        keyboard.add(
            InlineKeyboardButton(
                text=f"{status_device}|{device["label"]}|{session_status}",
...
async def list_for_copy_kb(list_device: list):
    ...
    for device in list_device:
        if device["session_status"] == "ACTIVE":
            ...
            keyboard.add(
                InlineKeyboardButton(
                    text=f"{status_device}|{device["label"]}",
                    copy_text=CopyTextButton(
                        text=f"{settings.REDIRECT_LINK}/{device['serial_number']}"
                    ),
                )
            )
